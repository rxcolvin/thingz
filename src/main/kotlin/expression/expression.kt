...
data class And(val exprs: List<BoolExpr>) : BoolExpr {
    constructor(left: BoolExpr, right: BoolExpr) :
            this(listOf(left, right))
...
data class Parameter<T>(val id: String) : Value<T> {
    override fun isNull(): Boolean = false
}
...
fun main() {
    println(
        toEnglish(
            And(
                Eq(Field("xxx"), Const(10)),
                Eq(Field("yyy"), Const(11))
            )
        )
    )
}
...
fun encode(test: Any): String {
    fun encodeConst(c: Const<*>) = quotize(c.value)
    fun encodeAnd(and: And): String = "And(" +
            and.exprs.map { encode(it) }.joinToString(separator = ", ") + ")"

    fun encodeField(field: Field<*>): String = "Field(${quotize(field.id)})"
